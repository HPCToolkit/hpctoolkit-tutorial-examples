   runqahpct/home/msi3/gits/hpctoolkit-tutorial-examples/examples/cpu/kripke     Mon May 27 14:45:21 CDT 2024     expt.1.realtime.t
Currently Loaded Modulefiles:
 ...
 8) py-pip/23.1.2       16) curl/8.7.1-gcc-11.4.0-l6pnvor          24) hpctoolkit/2024.01.stable/cuda11.8.0-rocm6.0.0  
   Begin QA runqahpct expt.1.realtime.t in directory /home/msi3/gits/hpctoolkit-tutorial-examples/examples/cpu/kripke
   Target executable = Kripke/Kripke/build/kripke.exe 
    hpcrun arguments = " -e REALTIME -t" 
     No GPU profiling requested
     Tracing is requested

 runqahpct Launching timeout -s6 90  /usr/bin/time -f"timefmt" hpcrun -o meas.expt.1.realtime.t  -e REALTIME -t Kripke/Kripke/build/kripke.exe  
...
======
  ...
  ----------------  ------------  ------------
  Generate                     1       0.00315
  LPlusTimes                  10       0.76836
  LTimes                      10       0.77017
  Population                  10       0.12348
  Scattering                  10       3.74088
  Solve                        1       7.45846
  Source                      10       0.00152
  SweepSolver                 10       1.93204
  SweepSubdomain             160       1.90429

TIMER_NAMES:Generate,LPlusTimes,LTimes,Population,Scattering,Solve,Source,SweepSolver,SweepSubdomain
TIMER_DATA:0.003149,0.768363,0.770169,0.123475,3.740882,7.458457,0.001518,1.932038,1.904293

Figures of Merit
================

  Throughput:         1.687066e+07 [unknowns/(second/iteration)]
  Grind time :        5.927449e-08 [(seconds/iteration)/unknowns]
  Sweep efficiency :  98.56396 [100.0 * SweepSubdomain time / SweepSolver time]
  Number of unknowns: 12582912

END
7.40s. user;  0.09s. system;  7.55s. elapsed;  99%%CPU (0avgtext+0avgdata 283008maxresident)k
   -------------------------------------- Data collection complete Mon May 27 14:45:29 CDT 2024 
    Target-runtime= 8 s. = 0m.:8s.; log.expt.1.realtime.t
 
SUMMARY: samples: 1489 (recorded: 1489, blocked: 0, errant: 0, trolled: 0, yielded: 0),
    No runtime errors found.
  Experiment hpcrun files: 1 Total, Target CPU threads = 1, Target GPU threads = 0
-rw-r--r-- 1 msi3 users 15472 May 27 14:45 meas.expt.1.realtime.t/kripke.exe-000000-000-2a803c80-3377401-0.hpcrun
  Experiment hpctrace files: Target CPU traces = 1, Target GPU traces = 0
-rw-r--r-- 1 msi3 users 17912 May 27 14:45 meas.expt.1.realtime.t/kripke.exe-000000-000-2a803c80-3377401-0.hpctrace
   --------------------------------------  

 runqahpct Launching /usr/bin/time -f"timefmt" hpcstruct  meas.expt.1.realtime.t  >> log.expt.1.realtime.t 2>&1
...
INFO: identifying load modules that need binary analysis
 ...
 begin parallel analysis of CPU binary libstdc++.so.6.0.25 (size = 1661416, threads = 2)
   ...
   end parallel analysis of CPU binary libstdc++.so.6.0.25 (Copied from cache)
   end parallel analysis of CPU binary kripke.exe (Copied from cache)

0.13s. user;  0.23s. system;  0.13s. elapsed;  264%%CPU (0avgtext+0avgdata 16904maxresident)k
    hpcstruct-runtime= 0 s. = 0m.:0s.; log.expt.1.realtime.t
  Experiment gpubins-used: 0
    No hpcstruct errors found.
   --------------------------------------  

 runqahpct Launching /usr/bin/time -f"timefmt" hpcprof  -o dbase.expt.1.realtime.t meas.expt.1.realtime.t  >> log.expt.1.realtime.t 2>&1
4.84s. user;  0.49s. system;  0.48s. elapsed;  1103%%CPU (0avgtext+0avgdata 26424maxresident)k
    hpcprof-runtime= 1 s. = 0m.:1s.; log.expt.1.realtime.t
    No hpcprof errors found.

   ==========================================================
     TESTPASS; -- 9 s. = 0m.:9s. ; see /home/msi3/gits/hpctoolkit-tutorial-examples/examples/cpu/kripke/log.expt.1.realtime.t
Mon May 27 14:45:30 CDT 2024
