...
echo "time ${HPCTOOLKIT_LAUNCHER} -n 4 -a 1 -g 1 hpcrun -o $OUT.m -e cycles -e gpu=nvidia -t ./lmp -k on g 1 -sf kk -in build/lammps/src/USER-INTEL/TEST/in.intel.lj"
time ${HPCTOOLKIT_LAUNCHER} -n 4 -a 1 -g 1 hpcrun -o $OUT.m -e cycles -e gpu=nvidia -t ./lmp -k on g 1 -sf kk -in build/lammps/src/USER-INTEL/TEST/in.intel.lj
